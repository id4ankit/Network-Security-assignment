...
struct ipheader {
    ...
    unsigned short int iph_offset; // Fragment offset
    ...
    if (ntohs(eth->ether_type) == 0x0800) { // 0x0800 is IPv4 type
        ...
        switch(ip->iph_protocol) {                               
            case IPPROTO_TCP:
                ...
                tcp = (struct sniff_tcp*)(packet + sizeof(struct ethheader) + size_ip);
                size_tcp = TH_OFF(tcp)*4;

                payload = (u_char *)(packet + sizeof(struct ethheader) + size_ip + size_tcp);
...
int main() {
    ...
    // Step 3: Apply the compiled filter
